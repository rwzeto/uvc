...
# spatial distributions that match data sheets
            ...
            for y in self.world_grid.axis[1]:
                for z in self.world_grid.axis[2]:
                    if np.abs(A*(x-x0) + B*(y-y0) + C*(z-z0)) < ZERO_THRESH:
                        nx, ny, nz = self.world_grid.iloc((x, y, z))
                        surface[nx][ny][nz] = SURFACE_VALUE
        return surface
    
    def calculate_flux(self, intensity_distribution):
        flux = 0
        for x in self.world_grid.axis[0]: 
            for y in self.world_grid.axis[1]:
                for z in self.world_grid.axis[2]:
                    ...
                    if self.data[nx][ny][nz] != 0:
                        flux += intensity_distribution.data[nx][ny][nz]*GRID_RESOLUTION**3
        return flux
...
class plot_gui:
    def __init__(self, intensity_distribution, plane):
        ...
        self.plane = plane
        plot_array = self.intensity_distribution.get_cross_section(x=-1)+self.plane.get_cross_section(x=-1)

        # cube for now
        extent_list = [-self.plane.world_grid.extent[0], self.plane.world_grid.extent[1], -self.plane.world_grid.extent[1], self.plane.world_grid.extent[1]]
        self.fig, self.ax = plt.subplots()
        self.im = self.ax.imshow(plot_array, origin='lower', extent=extent_list, cmap='plasma')
        self.im.set_clim(0, TOTAL_POWER*1.5)
        self.ax.set_xlabel("Y position")
        self.ax.set_ylabel("Z position")

        divider = make_axes_locatable(self.ax)
        cax = divider.append_axes("right", size="5%", pad=0.05)
        plt.colorbar(self.im, ax=self.ax, cax=cax, label="Intensity (mW/mm^2")
        plt.subplots_adjust(left=0.25, bottom=0.25)

        axfreq = plt.axes([0.25, 0.1, 0.65, 0.03])
        sfreq = Slider(axfreq, 'X position', -1, 1, valinit=-1, valstep=GRID_RESOLUTION)
        sfreq.on_changed(self.update_x)

        self.ax.set_title("Incident surface power = %.2f mW\n %d mm^2 surface, LED power=%.2f mW"%(self.plane.calculate_flux(self.intensity_distribution), GRID_MAX, TOTAL_POWER))
...
# cube for now
world = world_grid((GRID_MAX, GRID_MAX, GRID_MAX), GRID_RESOLUTION)

point_source = spherical(world, origin=(0, GRID_MAX, 0), normal=np.array([0, 0, 0, ]), total_power=TOTAL_POWER)
detector_plane = surface(world, origin=(0, GRID_MAX-SURFACE_DISTANCE, 0), normal=np.array([0, 1, 0])/np.sqrt(2))
